...
import { Users, Download, Search, Phone, Mail, Calendar } from 'lucide-react'
...
type LeadRow = Omit<Lead, 'campaign_name'> & {
  campaigns?: {
    name: string | null
  } | null
}
...
export default function Leads() {
  ...
  const exportCSV = () => {
    const headers = ['Name', 'Phone', 'Email', 'Status', 'Campaign', 'Date']
  ...
  return (
    <>
      <Helmet><title>Leads — LeadPilot</title></Helmet>

      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-white">Leads</h1>
            <p className="text-slate-400 text-sm mt-1">{leads.length} total leads from all campaigns</p>
          </div>
          {leads.length > 0 && (
            <button
              onClick={exportCSV}
        ...
        {loading ? (
          <div className="space-y-2">
            {[...Array(5)].map((_, i) => <div key={i} className="h-14 glass rounded-xl animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <p className="text-slate-500 text-sm max-w-sm mx-auto">
              {leads.length === 0
                ? 'Leads will appear here as soon as your campaign starts generating them.'
              ...
              {filtered.map(lead => (
                <div key={lead.id} className="grid grid-cols-12 gap-4 px-5 py-4 hover:bg-white/3 transition-all items-center">
                  <div className="col-span-4 flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-brand-700/30 flex items-center justify-center text-brand-300 text-xs font-bold flex-shrink-0">
                  ...
                  <div className="col-span-3">
                    <p className="text-slate-400 text-sm truncate">{lead.campaign_name}</p>
                  </div>

                  <div className="col-span-2">
                    <span className={cn(
                      'text-xs px-2.5 py-1 rounded-full border',
                      STATUS_CONFIG[lead.status]?.color ?? 'text-slate-400 border-white/10'
                    )}>
